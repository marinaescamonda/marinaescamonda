...
function(
	...
	return GalleryBase.extend({
		...
		resizeImage: function(targetImage, scale){
			...
			this.parentView.checkForChanges();
		...
		render: function () {
			...
			_.each(this.images, function(image, index){
				...
				model_data.meta_data[index].width = Math.max(5, model_data.meta_data[index].width)
				...
				var scaled_height = y_ratio * parseFloat(model_data.meta_data[index].width)
